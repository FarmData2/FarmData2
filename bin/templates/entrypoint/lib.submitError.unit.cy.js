...
describe('Test the %ENTRY_POINT_TITLE% lib submission error', () => {
  ...
  it(
    ...
    () => {
      /*
       * TODO: Create a error on submission of the last asset, log or quantity
       * that is created by %ENTRY_POINT%/lib.js. At that point all other
       * records should have been created and thus should also all be deleted
       * by their operation's `undo` function.
       */
      ...
      cy.wrap(
        lib
          ...
          .catch((error) => {
            ...
            expect(sampleOpCount).to.equal(1);
          }),
        { timeout: 10000 }
      );
    }
