...
Once you have [installed the FarmData2 Development Environment](../../INSTALL.md) and have a running instance of FarmData2 it might be useful to work through a quick tour. This tour will not cover every feature of FarmData2, but it will give you a feel for how it works, what it does and how it interfaces with farmOS.

## Creating a Tray Seeding

1. [Login to farmOS](http://farmos) and by using the [credentials for a farmOS Farm Manager](../install/working.md#farmos-credentials).
...
3. Complete the Tray Seeding form by using whatever values you want.
...
4. Click the "Submit" button.
   ...
   FarmData will show a <!-- vale RedHat.DoNotUseTerms = NO : the banner is clearly identified as at the top of the screen -->green<!-- vale RedHat.DoNotUseTerms = YES --> banner at the top of the screen with the text "Tray Seeding created." indicating that the records for the tray seeding have been created in farmOS.
...
For example, when the example tray seeding was created FarmData2 created:
...
5. Use the farmOS menu to select "Records -> Assets -> Plant."
   ...
   Some plant assets will be listed underneath the map. Most of these are the example data provided by the FarmData2 Development Environment for testing purposes.

6. Click the "ID" column header to sort the assets by their ID number. This will bring the Rhubarb tray seeding asset to the top of the list.
...
7. Click the "Asset Name" of the tray seeding asset that you created to see more details about the asset.
...
9. Click the "Log name" of for the seeding log that you created to display more details about the log.
...
Quantities are most often observed by looking at a log or an inventory value associated with an asset. But, they are independent records within farmOS and are created by FarmData2 code when the Tray Seeding is submitted. Thus, knowing how to access them in farmOS can be useful.
...
12. Use the "Crop" dropdown to select the crop that you tray seeded. The details of your tray seeding will appear in the table underneath the crop.

13. Select some number of, but not all, the trays to transplant from the "Trays" column.

14. Complete the rest of the visible parts of the "Transplanting Form."
...
16. Select some equipment and add some information about the soil disturbance.
    ...
    <a href="images/tour/RhubarbEquipment.png"><img src="images/tour/RhubarbEquipment.png" alt="Rhubarb Transplanting - Equipment and Soil Disturbance" width="450" style="border: 1px solid black"></a>

17. Click the "Submit" button.

    FarmData will show a <!-- vale RedHat.DoNotUseTerms = NO : the banner is clearly identified as at the top of the screen -->green<!-- vale RedHat.DoNotUseTerms = YES --> banner at the top of the screen with the text "Transplanting created." indicating that the records for the transplanting have been created in farmOS.

### More farmOS Assets, Logs and Quantities

When a transplanting is submitted FarmData2 creates _assets_, _logs_ and _quantities_ that represent the transplanted crop, the transplanting action, the soil disturbance and the associated quantities.
...
- A decrease in the <!-- RedHat.Definitions = NO -->TRAYS<!-- RedHat.Definitions = YES --> inventory on the original _plant asset_.
- An _activity log_ representing the action of transplanting the crop with quantities for:
  ...
  - The width of the beds where the plant was transplanted (_Bed Width_).
  - The number of rows in the beds (_Rows/Bed_). - An -activity log\_ representing the soil disturbance with quantities for:
  - The depth to which the soil was <!-- vale alex.ProfanityUnlikely = NO : it is soil that is physically disturbed here. -->disturbed<!-- vale alex.ProfanityUnlikely = Yes --> (_Depth_).
