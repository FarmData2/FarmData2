...
<pre>
FarmData2
├── <a href="#devcontainer-directory">.devcontainer</a>
├── <a href="#fd2-directory">.fd2</a>
├── <a href="#fd2dev-directory">.fd2dev</a>
│   └── db.conf
...
- The `.devcontainer` directory contains the files that define the FarmData2 Development Environment that is created when a Codespace is created on this repository. Detailed information about the contents of `.devcontainer` and the FarmData2 Development Environment can be found in the [FarmData2 Infrastructure Guide](../maintainers/infrastructure.md)

##### `.fd2` Directory

<!-- vale Microsoft.Vocab = NO -->

- The `.fd2` directory is used by the `bin/installDB.bash` script to download compressed releases of the sample database.
...
- The `.fd2dev` directory contains the `db.conf` file, which indicates the version of the sample database that is used by the branch. The `db.conf` file is used by the `post-checkout` Git hook to install the proper version of the sample database when switching between branches that might require different versions.
