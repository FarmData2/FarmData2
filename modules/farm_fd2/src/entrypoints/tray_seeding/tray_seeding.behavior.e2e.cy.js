...
describe('Check the behavior of the Tray Seeding page.', () => {
  ...
  it('Seeds/Cell does not have medium or large increments', () => {
    cy.get('[data-cy="seeding-seeds"]')
      .find('[data-cy="numeric-increase-md"]')
      .should('not.exist');
    cy.get('[data-cy="seeding-seeds"]')
  ...
  it('Add crop goes to correct form', () => {
    cy.get('[data-cy="seeding-crop-name"]')
    ...
    cy.url().should(
      'include',
      'admin/structure/taxonomy/manage/plant_type/add'
    );
  });

  it('Add location goes to correct form', () => {
    cy.get('[data-cy="seeding-location-name"]')
      .find('[data-cy="selector-add-button"]')
      .click();

    cy.url().should('include', 'asset/add/structure');
  });

  it('Add tray size goes to correct form', () => {
    cy.get('[data-cy="seeding-tray-size"]')
      .find('[data-cy="selector-add-button"]')
      .click();

    cy.url().should('include', 'admin/structure/taxonomy/manage/tray_size/add');
  });
});
